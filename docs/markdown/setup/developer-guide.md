...
- To use IAM accessKeyId and secretKey to access AWS Trino, you need to specify the following environment variables.
...
- [ Optional ] Log configurations
  ...
  1. `TRINO_LOG_PATH`=`<path_to_log_file>`(e.g.:`"C:\\Users\\BitQuillUser\\Desktop\\Trino ODBC Driver"`)
  ...
  The log path indicates the path to store the log file. The log file name is formatted as `trino_odbc_YYYYMMDD.log`, where `YYYYMMDD` (e.g., 20220225 <= Feb 25th, 2022) is the date at the first log message.

  2. `TRINO_LOG_LEVEL`=`<log_level>`. The default is `2` (means WARNING Level). Possible values:
     - 0: OFF
     - 1: ERROR
     - 2: WARNING
     - 3: INFO
...
5. On the Developer PowerShell, run one of the build scripts to create an initial compilation.
   ...
   2. Navigate to the `build\odbc\cmake` folder to use the generated solution file, `Trino-ODBC.sln` to work on
...
6. Open a **64-bit** command shell or **64-bit** PowerShell window, **as Administrator**, run the command below
   ```
   .\<repo-folder>\src\odbc\install\install_amd64.cmd <repo-folder>\build\odbc\cmake\Debug\trino.odbc.dll
   ```
...
8. Once configured, run the tests:
   - Run integration tests: `.\build\odbc\bin\<Release or Debug>\trino-odbc-integration-tests.exe`.
   - Run unit tests: `.\build\odbc\bin\<Release or Debug>\trino-odbc-unit-tests.exe`.

### Known issues

Driver installer detects existing driver and isn't allowing installation

1.  Go to `Apps & Features` settings page and search for `Trino ODBC Driver`. If there is already a Trino ODBC driver installed, manually uninstall the driver (optional to uninstall C++ Redistributable, for details see [Windows installation guide](windows-installation-guide#microsoft-visual-c-redistributable)), and then attempt installing again.
2.  If the driver is not showing under `Apps & Features`, remove driver registry `Computer\HKEY_LOCAL_MACHINE\SOFTWARE\ODBC\ODBCINST.INI\Amazon Trino ODBC Driver` using [Registry Editor](https://support.microsoft.com/en-us/windows/how-to-open-registry-editor-in-windows-10-deab38e6-91d6-e0aa-4b7c-8878d9e07b11), and then attempt installing again.
...
2. Run one of the build scripts to create an initial compilation.
   1. E.g.: from the root of the Trino ODBC repository, run `./build_mac_release64.sh`
   2. The generated driver files will be placed in the `build/odbc/lib` folder.
3. Set the environment variable `ODBCINSTINI`. On a developer's machine, set it to `<repo-folder>/build/odbc/lib/trino-odbc-install.ini`.
...
7. Once configured, run the tests:
   - Run integration tests: `./build/odbc/bin/trino-odbc-integration-tests --catch_system_errors=false`.
   - Run unit tests: `./build/odbc/bin/trino-odbc-unit-tests  --catch_system_errors=false`.
...
```
message: 01000: [unixODBC][Driver Manager]Can't open lib 'Amazon Trino ODBC Driver' : file not found
```
...
By default, the Trino ODBC driver reports databases as catalogs. This behavior is matching with Trino ODBC Driver version 1.0.0, and it is due to Excel on macOS not showing tables with duplicate names when databases are reported as schemas.
...
1. Follow below instructions to load sample data from Trino.

   i. To load `SampleDB.IoTMulti`: Go to **Trino** → **Databases**. Click `Create database` and you’ll see the below page. Select `Sample database` and enter `SampleDB` as database name. Check `IoT` and select `Multi-measure records`. Finished page should look like below.

   ![](../images/sampleDB-create.png)

   Then clicking on "Create Database" will create the desired database.

   ii. To load `meta_queries_test_db.IoTMulti` and `meta_queries_test_db.DevOpsMulti`: Go to **Trino** → **Databases**. Click `Create database` and you’ll see the below page. Select `Sample database` and enter `meta_queries_test_db` as database name. Check `IoT` and `DevOps` and select `Multi-measure records`.

   ![](../images/meta_queries_test_db-create.png)

   Then clicking on "Create Database" will create the desired database.

2. Run below commands to load the test data. The commands provided work on Linux and Windows Command Prompt, please modify the command to work with your desired platform. For help around using quotation marks in different platforms, please visit [AWS official guide for using quotation marks](https://docs.aws.amazon.com/cli/latest/userguide/cli-usage-parameters-quoting-strings.html). For general help with AWS CLI commands, please visit [Amazon Trino official CLI guide](https://docs.aws.amazon.com/trino/latest/developerguide/Tools.CLI.html) and [AWS CLI general guide](https://docs.aws.amazon.com/cli/latest/userguide/cli-chap-using.html).
...
3. Load large scale data with data writer.
   ...
   iii. Run below `trino-populate-data` command to insert test data for big table tests. For help with this command, see [data-population guide](data-population-guide.md). Note that this command will write data in region `us-west-2`, which is expected by our tests.

   ```
   trino-populate-data -u <access_key_id> -p <secret_access_key> -d data_queries_test_db -t TestMultiMeasureBigTable -ty computer -l 20000
   ```
...
Reading and writing data on Trino requires corresponding permissions. For read permissions, it is suggested to add Amazon-managed policy `AmazonTrinoReadOnlyAccess`. For write permissions, see below for example policy.
...
#### `trino-populate-data` command not found on PowerShell

`trino-populate-data` is a data population tool provided together with Trino ODBC driver. It is under `<your_trino_repository>\build\odbc\bin\<Debug or Release>`, add that path to your environment variable PATH to use it without path.
...
2. For AAD integration tests to be run successfully, you need to specify the following environment variables. For instructions on how to set-up and find the AAD connection property values, go to the [SAML 2.0 Azure AD set up guide](/docs/markdown/setup/aad-saml-setup.md#trino-odbc-dsn-configuration).
...
5. Start DSN window and create a connection to Trino. Click 'Test' button to verify.
